...
/// Settle a wager that has received VRF randomness
/// This is called after the minimal VRF callback has stored the result
#[derive(Accounts)]
pub struct SettleWagerAccountConstraints<'info> {
    ...
    #[account(
        ...
        bump = wager.bump,
        constraint = wager.status == WagerStatus::ReadyToSettle @ DiceDuelError::InvalidWagerStatus,
        constraint = wager.vrf_result.is_some() @ DiceDuelError::VrfResultNotAvailable,
    )]
...
pub fn handle_settle_wager(context: Context<SettleWagerAccountConstraints>) -> Result<()> {
    ...
    // Get the VRF result (should exist due to constraint)
    let result = wager.vrf_result.unwrap();
